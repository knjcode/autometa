(function() {
  var HORIZONTAL_MARK, TEMPLATES_DIRS, VERTICAL_MARK, decodeCell, decodeCol, decodeRow, ejs, encodeCell, encodeCol, encodeRow, file_dir, fs, getTemplateFilePath, mapKey, moveCell, parseCsv, path, readExcelFile, readlineSync, registerTemplate, specified_template_id, splitCell, templates_dirs, valid, xls, xlsx;
  ...
  registerTemplate = function(template, filename, overwrite) {
    var answer, basename, err, targetFile, template_dir, _i, _len;
      ...
      if (fs.existsSync(template)) {
        ...
        if ((ext === '.csv') || (ext === '.ejs')) {
          if (!registerTemplate(template, filename, overwrite)) {
            _results.push(console.error('Failed to generate file.'));
          } else {
            _results.push(void 0);
          }
        } else {
          _results.push(console.error('Error. ' + filename + ' is not template.'));
        }
      } else {
        _results.push(console.error('Error. Input file does not exist.'));
      }
    }
    return _results;
  };
  ...
  exports.generateFile = function(excelfile, filename, overwrite) {
    var FileName, answer, basename, err, i, out, output, outputs, sheetnum, _i;
