(function() {
  ...
  registerTemplate = function(template, filename) {
    var err, targetFile, template_dir, _i, _len;
      ...
      if (fs.existsSync(template)) {
        ...
        if (ext === '.csv' || ext === '.ejs') {
          if (!registerTemplate(template, filename)) {
            _results.push(console.error('Error. Can not write file.'));
          } else {
  ...
  exports.generateFile = function(excelfile, filename) {
    var FileName, err, i, out, output, outputs, sheetnum, _i;
